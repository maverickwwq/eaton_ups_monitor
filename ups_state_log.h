...
_Bool conditionChangeLog(int exceptionCode,_Bool exceptionSolved,int upsNo){
	extern const char *frames[];
	time_t time_seconds;
	struct tm *t;
	time_seconds=time(NULL);
	t=localtime(&time_seconds);
	FILE *logFile=fopen("ups_log.txt","a+");//a+ 以附加方式打开可读写的文件。若文件不存在，则会建立该文件，
